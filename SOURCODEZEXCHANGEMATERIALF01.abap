...
FORM post_bapi .
  ...
  OR gs_po_header-ekgrp IS INITIAL.
    MESSAGE 'Thi#u d# li#u header (Vendor/Company/Org/Group).' TYPE 'S' DISPLAY LIKE 'E'.
    RETURN.
  ENDIF.
  ...
  IF lt_poitem IS INITIAL.
    MESSAGE 'Không có dòng h#p l# ## t#o PO.' TYPE 'S' DISPLAY LIKE 'E'.
    RETURN.
  ENDIF.
  ...
  IF lv_success = abap_true AND lv_po_number IS NOT INITIAL.
    ...
    MESSAGE |PO { lv_po_number } t#o thành công.| TYPE 'S'.
    SET PARAMETER ID 'BES' FIELD lv_po_number.
    CALL TRANSACTION 'ME23N' AND SKIP FIRST SCREEN.
  ELSE.
    CALL FUNCTION 'BAPI_TRANSACTION_ROLLBACK'.
    MESSAGE 'T#o PO th#t b#i. Xem l#i # trên (RETURN).' TYPE 'E'.
  ENDIF.
...
*      lt_poitmtext  TYPE STANDARD TABLE OF bapimepotextitem,
  ...
  IF lt_item IS INITIAL AND lt_itemx IS INITIAL
   AND lt_schedule IS INITIAL AND lt_schedulex IS INITIAL.
    MESSAGE 'Không có thay ##i nào ## c#p nh#t PO.' TYPE 'S'.
    RETURN.
  ENDIF.
  ...
  IF lv_ok = abap_true.
    CALL FUNCTION 'BAPI_TRANSACTION_COMMIT' EXPORTING wait = 'X'.
    MESSAGE |PO { lv_ebeln_norm } #ã ###c c#p nh#t.| TYPE 'S'.
  ELSE.
    CALL FUNCTION 'BAPI_TRANSACTION_ROLLBACK'.
    MESSAGE |C#p nh#t PO { lv_ebeln_norm } th#t b#i. Xem l#i # trên.| TYPE 'E'.
  ENDIF.
...
FORM post_bapi_receipt.
  ...
  LOOP AT gt_receipt_item ASSIGNING <ls_rec>.
    ...
    IF sy-subrc <> 0.
      MESSAGE |Dòng { sy-tabix }: Không tìm th#y thông tin trong EKPO cho PO { <ls_rec>-purchase_order } và Item { <ls_rec>-item }| TYPE 'E'.
      CONTINUE.
    ENDIF.
    ...
    IF ls_item-material IS INITIAL.
      MESSAGE |Dòng { sy-tabix }: Thi#u Material Number cho PO { <ls_rec>-purchase_order } và Item { <ls_rec>-item }| TYPE 'E'.
      CONTINUE.
    ENDIF.
...
FORM post_bapi_invoice .
  ...
  ls_head-header_txt    = 'Credit Memo theo GR'.
  ...
  IF lt_item IS INITIAL.
    MESSAGE 'Không có dòng nào h#p l# ## post Credit Memo.' TYPE 'S' DISPLAY LIKE 'E'.
    RETURN.
  ENDIF.
  ...
  ELSE.
    CALL FUNCTION 'BAPI_TRANSACTION_ROLLBACK'.
    MESSAGE 'Post Credit Memo th#t b#i. Xem log # trên.' TYPE 'E'.
  ENDIF.
...
*&---------------------------------------------------------------------*
*& Form recalc_balance_300
*&---------------------------------------------------------------------*
FORM recalc_balance_300
  CHANGING ev_balance TYPE bapi_incinv_create_header-gross_amount.

  DATA: lv_sum_items TYPE bapi_incinv_create_header-gross_amount VALUE 0,
        lv_tax_amt   TYPE bapi_incinv_create_header-gross_amount VALUE 0,
        lv_hdr_amt   TYPE bapi_incinv_create_header-gross_amount,
        lv_tax_code  TYPE mwskz,
        lv_rc        TYPE sy-subrc.

  " 1) Sum item amounts (your grid amounts should be NET per line)
  LOOP AT gt_invoice_item INTO DATA(ls_item).
    lv_sum_items = lv_sum_items + ls_item-amount.
  ENDLOOP.

  lv_hdr_amt = gs_invoice_header-zamount.

  " 2) Derive tax amount:
  "    - If header already has a tax amount, use it.
  "    - Else try to calculate from tax code (header > single distinct line code).
  IF gs_invoice_header-tax_amount IS NOT INITIAL.
    lv_tax_amt = gs_invoice_header-tax_amount.
  ELSE.
    lv_tax_code = gs_invoice_header-tax_code.

    IF lv_tax_code IS INITIAL.
      "If all lines share one tax code, reuse it
      DATA lt_codes TYPE SORTED TABLE OF mwskz WITH UNIQUE KEY table_line.
      LOOP AT gt_invoice_item INTO ls_item WHERE tax_code IS NOT INITIAL.
        INSERT ls_item-tax_code INTO TABLE lt_codes.
      ENDLOOP.
      IF lines( lt_codes ) = 1.
        READ TABLE lt_codes INDEX 1 INTO lv_tax_code.
      ENDIF.
    ENDIF.

    IF lv_tax_code IS NOT INITIAL
       AND gs_invoice_header-bukrs IS NOT INITIAL
       AND gs_invoice_header-zcurrency IS NOT INITIAL
       AND gs_invoice_header-budat IS NOT INITIAL.

      "=== Use FI tax calculator (same logic MIRO uses) ===
      " Net # Tax. If you keep grid 'amount' as net per line, this is correct.
      CALL FUNCTION 'FI_TAX_CALC_FROM_NET_AMOUNT'
        EXPORTING
          i_bukrs      = gs_invoice_header-bukrs
          i_mwskz      = lv_tax_code
          i_budat      = gs_invoice_header-budat
          i_waers      = gs_invoice_header-zcurrency
          i_net_amount = lv_sum_items
        IMPORTING
          e_ftxa       = lv_tax_amt     " total tax amount
        EXCEPTIONS
          OTHERS       = 1.
      lv_rc = sy-subrc.

      " Fallback: if calculator not available/configured, keep tax = 0 (or header value).
      IF lv_rc <> 0.
        CLEAR lv_tax_amt.
      ENDIF.
    ENDIF.
  ...
  " 3) Final MIRO-style balance:
  "    Balance = Header Gross - (Sum of item nets + Tax)
  ev_balance = lv_hdr_amt - ( lv_sum_items + lv_tax_amt ).
