...
FORM get_short_text .
  ...
  SELECT ekpo~matnr, ekpo~netpr, ekko~waers, ekpo~peinh, mara~meins, mara~matkl ,ekpo~infnr
  FROM ekpo
  ...
  LOOP AT gt_po_item ASSIGNING <fs_item>.
    ...
    IF sy-subrc = 0.
      ...
      <fs_item>-matkl = ls_po_item1-matkl.
    ENDIF.
  ENDLOOP.
  PERFORM alv_refresh USING 'GO_GRID_01'.
ENDFORM.
...
FORM get_data_200.
  ...
  LOOP AT lt_ekpo INTO ls_ekpo.
    ...
    " Default GR
    IF ls_ekpo-retpo = 'X'.
      gs_receipt_item-movement_type = '161'.  " Movement Type cho Return PO
    ELSE.
      gs_receipt_item-movement_type = '101'.  " Movement Type cho GR (Stock Entry)
    ENDIF.
...
FORM get_po_history .
  ...
  " 1) Require vendor input
  IF p_hist IS INITIAL.
    MESSAGE 'Please enter a Vendor number.' TYPE 'S' DISPLAY LIKE 'E'.
    RETURN.
  ENDIF.

  " 2) Normalize LIFNR
  DATA(lv_lifnr) = |{ p_hist ALPHA = IN }|.
  p_hist = lv_lifnr.
  ...
  " 4) No data for this vendor (bad vendor or no POs)
  IF gt_po_hist IS INITIAL.
    MESSAGE 'Incorrect vendor or there is no purchase order number.' TYPE 'S' DISPLAY LIKE 'E'.
    RETURN.
  ENDIF.

  SORT gt_po_hist BY erdat DESCENDING ebeln ebelp.
  gv_alv_mode = 'POHIS'.
...
FORM get_material_docs .
  ...
  " 1) Require vendor
  IF p_mdoc IS INITIAL.
    MESSAGE 'Please input a Vendor number.' TYPE 'S' DISPLAY LIKE 'E'.
    RETURN.
  ENDIF.

  " 2) Normalize vendor (ALPHA IN)
  DATA(lv_lifnr) = |{ p_mdoc ALPHA = IN }|.
  p_mdoc = lv_lifnr.

  " 3) Select Material Documents for that vendor (via PO header)
  "    + join EKPO to get PO item Short Text (TXZ01)
  SELECT mk~mblnr, mk~mjahr, mk~budat, mk~cpudt, mk~cputm, mk~usnam, mk~tcode,
         ms~zeile, ms~matnr, ms~werks, ms~lgort, ms~bwart, ms~menge, ms~meins,
         ms~ebeln, ms~ebelp, ek~lifnr,
         ep~txz01 AS short_text         " <— NEW
    FROM mseg AS ms
    INNER JOIN mkpf AS mk
      ON mk~mblnr = ms~mblnr AND mk~mjahr = ms~mjahr
    LEFT  OUTER JOIN ekko AS ek
      ON ek~ebeln = ms~ebeln
    LEFT  OUTER JOIN ekpo AS ep
      ON ep~ebeln = ms~ebeln AND ep~ebelp = ms~ebelp   " <— NEW
   WHERE ek~lifnr = @lv_lifnr
     AND ms~bwart IN ('101','161','122')
    INTO CORRESPONDING FIELDS OF TABLE @gt_matdoc.

  IF gt_matdoc IS INITIAL.
    MESSAGE 'Incorrect vendor or there''s no material document.' TYPE 'S' DISPLAY LIKE 'E'.
    RETURN.
  ENDIF.

  " 4) Fallback text from material master if PO text is empty
  DATA lt_need TYPE SORTED TABLE OF matnr WITH UNIQUE KEY table_line.
  LOOP AT gt_matdoc ASSIGNING FIELD-SYMBOL(<r>) WHERE short_text IS INITIAL AND matnr IS NOT INITIAL.
    INSERT <r>-matnr INTO TABLE lt_need.
  ENDLOOP.
  ...
  IF lt_need IS NOT INITIAL.
    SELECT matnr, maktx
      ...
      WHERE matnr = @lt_need-table_line
        AND spras = @sy-langu.

    LOOP AT gt_matdoc ASSIGNING <r> WHERE short_text IS INITIAL AND matnr IS NOT INITIAL.
      READ TABLE lt_makt ASSIGNING FIELD-SYMBOL(<tx>) WITH KEY matnr = <r>-matnr.
      IF sy-subrc = 0.
        <r>-short_text = <tx>-maktx.
      ENDIF.
    ENDLOOP.
  ENDIF.

  " 5) Sort by MATERIAL (ascending) for the ALV
  SORT gt_matdoc BY matnr ASCENDING mblnr ASCENDING zeile ASCENDING.  " <— CHANGED

  " Switch ALV to Material Document mode
  gv_alv_mode = 'MDOC'.
